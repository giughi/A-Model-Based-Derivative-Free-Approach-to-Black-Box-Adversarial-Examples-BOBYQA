...
from Attack_Code.BOBYQA.BOBYQA_Attack_BLOCK_STL import BlackBox_BOBYQA
...
for L_inf in L_inf_classes:
    for k in range(5,nimage):
        img = x_test[k, np.newaxis]

        tf.reset_default_graph()
        
        session_conf = tf.ConfigProto(intra_op_parallelism_threads=6,
        ...
        with tf.Session(config=session_conf) as sess:
            ...
            for target in targets:
                ...
                with open(results_dir+'BLOCK_attack_L_inf_'+str(L_inf/2)+'_max_eval_'+ str(3000) +'_one_round_2.txt', "wb") as fp:
                    pickle.dump(legend, fp)
                    print('SAVED at ', results_dir+'BLOCK_attack_L_inf_'+str(L_inf/2)+'_max_eval_'+ str(3000) +'_one_round.txt')
        del model
        K.clear_session()

        # steps = [0, 1,  4, 13, 40, 70, 100]
                # dimen = [3, 6,  12, 24, 20, 35, 50]