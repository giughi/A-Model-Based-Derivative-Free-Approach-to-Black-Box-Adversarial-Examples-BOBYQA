...
with open(main_dir+'/Results/Imagenet/Iterative_0.1_batch_dim_50_max_queries_8000_hier_True.txt', "rb") as fp:
    L_y = pickle.load(fp)
with open(main_dir+'/Results/Imagenet/Iterative_0.1_batch_dim_50_max_queries_8000_hier_False.txt', "rb") as fp:
    L_n = pickle.load(fp)
...
for i in range(len_y):
    for iteration in range(len(L_y[1][i])):
        temp = L_y[1][i][iteration]['fvals'].values
...
for i in range(len_n):
    for iteration in range(len(L_n[1][i])):
        temp = L_n[1][i][iteration]['fvals'].values
        for j in range(len(temp)):
