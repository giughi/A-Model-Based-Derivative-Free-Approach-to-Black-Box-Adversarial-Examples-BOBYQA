...
class GenAttack2(object):
    def mutation_op(self,  cur_pop, idx, step_noise=0.01, p=0.005):
        ...
        mutated_pop = perturb_noise * tf.cast(tf.random_uniform(cur_pop.get_shape()) < p, tf.float32) + cur_pop
        return mutated_pop


    def attack_step(self, idx, success, orig_copies, cur_noise, prev_elite, best_win_margin, cur_plateau_count, num_plateaus):
        if self.resize_dim:
            noise_resized = tf.image.resize_bilinear(cur_noise, (96,96))
        else:
            noise_resized = cur_noise
        noise_dim = self.resize_dim or 96
        cur_pop = tf.clip_by_value(noise_resized + orig_copies, self.box_min, self.box_max)
        time_befor = time.time()
        pop_preds = self.model.model(cur_pop)
        print('############## type of pop_prds' , type(pop_preds))
        print('############## shape of pop_preds', pop_preds.shape)
        print('############## the input was', cur_pop.shape)
        print('############## the input was', type(cur_pop))
        ...
        win_margin = tf.reduce_max(pop_preds[:, tf.cast(self.target, tf.int32)] - tf.reduce_max(pop_preds, axis=1))
        ...
        elite_idx = tf.cond(
            tf.equal(success,1),
            true_fn=lambda: tf.expand_dims(tf.cast(tf.argmax(success_pop), tf.int32), axis=0),
        ...
        print('###########################', select_probs)
        parents = tf.contrib.distributions.Categorical(probs=select_probs).sample(2*self.pop_size-2)
        ...
        pp2 = pp2 / (pp1+pp2)
        pp2 = tf.tile(tf.expand_dims(tf.expand_dims(tf.expand_dims(pp2, 1), 2), 3), (1, noise_dim, noise_dim,3))
        xover_prop = tf.cast(tf.random_uniform(shape=parent1.get_shape()) > pp2, tf.float32)
        childs = parent1 * xover_prop + parent2 * (1-xover_prop)
        idx = tf.Print(idx+1, [idx, tf.reduce_min(loss), win_margin, step_p, step_noise, new_cur_plateau_count,time_after-time_befor])
#         margin_log = tf.concat([margin_log, [[win_margin]]], axis=0)
        ...
        return idx, success, orig_copies, new_pop, tf.reshape(elite,(noise_dim, noise_dim, 3)), new_best_win_margin, new_cur_plateau_count, new_num_plateaus
    ...
    def __init__(self, model, pop_size=6, mutation_rate=0.001,
        ...
        self.input_img = tf.Variable(np.zeros((1, 96,96,3), dtype=np.float32), name='x', dtype=tf.float32)
        # copies of original image
        self.pop_orig = tf.Variable(np.zeros((self.pop_size, 96, 96, 3), dtype=np.float32), name='pop_orig', dtype=tf.float32)
        self.pop_noise = tf.Variable(np.zeros((self.pop_size, noise_dim, noise_dim, 3), dtype=np.float32), name='pop_noise', dtype=tf.float32)

        self.target = tf.Variable(0, dtype=tf.int64, name='target')
        self.init_success = tf.Variable(0, dtype=tf.int32, name='success')
        self.box_min = tf.tile(tf.maximum(self.input_img-eps, -1), (self.pop_size, 1,1,1))
        self.box_max = tf.tile(tf.minimum(self.input_img+eps, 1), (self.pop_size, 1,1,1))
#         self.margin_log = tf.Variable(initial_value=np.zeros((1,1), dtype=np.float32), validate_shape=False, name='margin_log', dtype=tf.float32)
#         self.margin_log.set_shape((None,1))
        self.tlab = tf.contrib.layers.one_hot_encoding([self.target], num_classes=10)
        self.i = tf.Variable(0, dtype=tf.int64, name='step')

        # Variables to detect plateau
        self.best_win_margin = tf.Variable(-1, dtype=tf.float32, name='cur_margin')
        ...
        cond = lambda  i, success, pop_orig, pop_noise, cur_elite, best_win_margin, cur_plateau_count, num_plateaus: tf.logical_and(tf.less_equal(i, self.max_steps), tf.equal(success, 0))


        attack_body = lambda i, success, pop_orig, pop_noise, cur_elite,  best_win_margin, cur_plateau_count, num_plateaus: self.attack_step(i, success, pop_orig, pop_noise, cur_elite, best_win_margin, cur_plateau_count, num_plateaus)
        
        
        self.attack_main = tf.while_loop(cond, attack_body, [self.i, self.init_success,  self.pop_orig,
             self.pop_noise, self.pop_noise[0], self.best_win_margin, self.cur_plateau_count,
             self.num_plateaus])
    ...
    def initialize(self, sess, img, target):
        ...
        init_noise = self.mutation_op(self.pop_noise, idx=self.i, p=self.mutation_rate, step_noise=self.eps)
#         sess.run(tf.assign(self.margin_log, np.zeros((1,1), dtype=np.float32)))
        sess.run(tf.assign(self.pop_noise, init_noise))
        sess.run(tf.assign(self.best_win_margin, np.array(-1.0, dtype=np.float32)))
    ...
    def attack_batch(self, sess, input_img, target_label):
        self.initialize(sess, input_img, target_label)
        (num_steps, success,  copies, final_pop, adv_noise, log_hist,_,_) = sess.run(self.attack_main)
        if success:
            if self.resize_dim:
                adv_img = sess.run(
                        tf.clip_by_value(
                            np.expand_dims(input_img, axis=0)+tf.image.resize_bilinear(np.expand_dims(adv_noise, axis=0), (96,96)),
            ...
            return adv_img[0], query_count, log_hist
        else:
            return input_img, self.max_steps*5, log_hist
