...
class BlackBox_BOBYQA:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
        ...
        self.max_extr = tf.Variable(0.,dtype = tf.float32)
        self.min_extr = tf.Variable(0.,dtype = tf.float32)
        self.mean_ext = tf.Variable(0., dtype=tf.float32)
        # self.assign_extremums = tf.placeholder(tf.float64, num_labels)
        # self.max_extr.assign(tf.reduce_max(self.assign_extremums))
        # self.min_extr.assign(tf.reduce_min(self.assign_extremums))
        # self.setup2 = []
        # self.setup2.append(self.max_extr.assign(tf.reduce_max(self.assign_extremums)))
        # self.setup2.append(self.min_extr.assign(tf.reduce_min(self.assign_extremums)))
        ...
        self.min = tf.reduce_min((1-self.tlab)*self.output,1)
        self.use_max = False
        ...
        if self.TARGETED:
            ...
            self.loss_res = tf.log(self.output) - (self.tlab*self.max_extr +
                                                   (1-self.tlab)*(self.min_extr + self.max_extr)/(2*tf.to_float(
                                                                                                 tf.size(self.other))))
            # self.loss_res = [self.distance[0], self.loss[0]]
            self.loss_qad = tf.reduce_sum(tf.square(self.loss_res))
    ...
    def blackbox_optimizer_ordered_domain(self, iteration,ord_domain, npt, maxfun):
        # build new inputs, based on current variable value
        var = self.real_modifier.copy()
        var_size = self.real_modifier.size
        ...
        opt_fun = lambda c: self.sess.run((self.loss_res),feed_dict = {self.use_log: self.use_log2,
                            self.modifier: vec2mod(c,indice,var)})[0]
        # if self.use_max:
        ...
        soln = dfols.solve(opt_fun, x_o, rhobeg = np.min(b-a)/2,
                          bounds = (a,b),maxfun=self.batch_size*1.2,
                          rhoend = np.min(b-a)/6,
                          npt=self.batch_size+1)
        ...
        return distance[0], evaluations + 1, nimgs        
    ...
    def attack_batch(self, img, lab):
        ...
        previous_loss = 1e6
        count_steady  = 0
        self.sess.run(tf.assign(self.min_extr, 0))
        self.sess.run(tf.assign(self.max_extr, 1))
        self.sess.run(tf.assign(self.mean_ext, 0))
        for step in range(self.MAX_ITERATIONS):
            ...
            if step%(self.print_every) == 0:
                loss, output, distance,quad_loss = self.sess.run((self.loss,self.output, self.distance,self.loss_qad),
                                                                 feed_dict={self.use_log : self.use_log2,self.modifier:
                                                                     self.real_modifier})
                print("[STATS][L2] iter = {}, cost = {}, time = {:.3f}, size = {}, loss = {:.5g}, loss_f = {:.5g}, maxel = {}, loss_wuad = {:.5g}, F_t = {:.5g}".format(step, eval_costs, train_timer, self.real_modifier.shape, distance[0],loss[0],np.argmax(output[0]),quad_loss,output[0][self.target]))
                sys.stdout.flush()
                # if step == 0:
                #     # self.sess.run(x.initializer for x in [self.min_extr, self.max_extr])
                #     print('Log minimum:', np.min(np.log(output[0])))
                #     print('Log maximum:', np.max(np.log(output[0])))
                #     print('Log median:', np.median(np.log(output[0])))
                #     print('minimum:', np.min(output[0]))
                #     print('maximum:', np.max(output[0]))
                #     print('median:', np.median(output[0]))
                #     self.sess.run(tf.assign(self.min_extr, np.min(np.log(output[0]))))
                #     self.sess.run(tf.assign(self.max_extr, np.max(np.log(output[0]))))
                #     self.sess.run(tf.assign(self.mean_ext, np.median(np.log(output[0]))))
            ...
            if self.ordered_domain:  
                ...
                if self.use_resize:
                    ...
                    if step == 120:
                        ...
                        iteration_domain = 0
                        force_renew = True
                        npt = 2*self.batch_size + 1
                        maxfun = 2.5*self.batch_size
                ...
                if (np.mod(iteration_scale,30)==0) or force_renew:
                ...
                l, evaluations, nimg = self.blackbox_optimizer_ordered_domain(iteration_domain,ord_domain,npt,maxfun)
            else:
