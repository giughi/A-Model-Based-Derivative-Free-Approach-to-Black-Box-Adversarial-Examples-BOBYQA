...
def vec2modMatRand3(c, indice, var, RandMatr, depend, b, a, overshoot):
    ...
    for i in range(n):
        ...
        if overshoot:
            if c[i]>0:
                temp[indices] += c[i]*np.max((b).reshape(-1, )[indices])/2 
            else:
                temp[indices] += np.abs(c[i])*np.min((a).reshape(-1, )[indices])/2 
        else:
    ...
    def blackbox_optimizer_ordered_domain(self, iteration, ord_domain, Random_Matrix, super_dependency, img, k, img0):
        ...
        if self.use_resize:
            ...
            for i in range(nn):
                ...
                a[i] = -1   #-4*self.L_inf # *np.min(down)
                b[i] = 1    #4*self.L_inf # 1.5*np.max(up)
                max_ind = np.argmax(up-down)
                xs =  np.divide( -(up+down),
                                (up-down))
                # x_o[i] = xs[max_ind]
                x_o[i] = 0*np.clip(xs[max_ind],-1,1)
        ...
        user_params = {'init.random_initial_directions':True, 
                       'init.random_directions_make_orthogonal':True}
