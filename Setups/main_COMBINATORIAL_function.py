...
#         logger = utils.ResultLogger(FLAGS.output_dir, FLAGS.flag_values_dict())
    if FLAGS.print_image:
        saving_dir = main_dir+'/Results/Imagenet/Adv_Images/COMBI_'+str(FLAGS.epsilon)+'_targeted_'+str(FLAGS.targeted)
        num_valid_images = 5
    else:
        if FLAGS.worst_attack:
            saving_dir = main_dir+'/Results/Imagenet/COMBI_'+str(FLAGS.epsilon)+'_function_jumping_'+str(FLAGS.jumping)+'_WORST_ATTACK.txt'
            print(saving_dir)
        else:
            saving_dir = main_dir+'/Results/Imagenet/COMBI_'+str(FLAGS.epsilon)+'_function_jumping_'+str(FLAGS.jumping)+'.txt'
        already_done = 0
