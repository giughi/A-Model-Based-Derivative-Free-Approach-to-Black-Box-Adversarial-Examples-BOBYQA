...
from Setups.Data_and_Model.setup_inception_2_first_tests import ImageNet, InceptionModel
...
#         logger = utils.ResultLogger(FLAGS.output_dir, FLAGS.flag_values_dict())
    ...
    summary_dir = main_dir+'/Results/Imagenet/summary_boby_combi_attack_in_first_bacth.txt'
    saving_dir = main_dir+'/Results/Imagenet/Reproducing_COMBI_First_batch_eps_'+str(FLAGS.epsilon)+'.txt'
    already_done = 0
    if os.path.exists(summary_dir):
        if os.path.getsize(summary_dir)>0:
            with open(summary_dir, "rb") as fp:
                attacks_sum = pickle.load(fp)
    ...
    for ii in range(already_done, num_valid_images):
        ...
        if real_label != attacks_sum[reproduced][0]:
            print('We have ',real_label, ' while it should be ',  attacks_sum[reproduced][0])
            continue
        reproduced += 1

        if attacks_sum[reproduced-1][3]>=15000:
            print('We have no convergence on boby')
            continue
        steps_done = -1
        perturbed_img = 0
        ord_domain = 0
