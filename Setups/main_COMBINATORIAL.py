...
# from Attack_Code.Combinatorial.tools.inception_v3_imagenet import model
...
# Directory
parser.add_argument('--asset_dir', default=main_dir+'/Attack_Code/Combinatorial/assets', type=str)
...
if __name__ == '__main__':
  ...
  with tf.Session() as sess:
      ...
      # Build a graph
      x_input = tf.placeholder(dtype=tf.float32, shape=(None, 299, 299, 3))
      y_input = tf.placeholder(dtype=tf.int32, shape=[None])

      model_inc = InceptionModel(sess, use_log=True)

      logits = model_inc.predict(x_input)
      ...
      while count < args.sample_size:
        ...
        print(type(initial_img[0,0,0,0]))
        print(initial_img)
        attack_lab = np.array([0]).astype(np.int32)
        print(type(attack_lab[0]))
        predic = sess.run(preds, feed_dict={x_input: initial_img})[0]
        print(predic)
        print(model_inc.predict(initial_img))
        modelll = sess.run([logits],
                           feed_dict={x_input: initial_img, y_input: attack_lab})
        print(modelll)
