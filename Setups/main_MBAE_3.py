...
#         logger = utils.ResultLogger(FLAGS.output_dir, FLAGS.flag_values_dict())
    if FLAGS.print_image:
        saving_dir = main_dir+'/Results/Imagenet/Adv_Images/BOBY_'+str(FLAGS.eps)+'_targeted_'+str(FLAGS.targeted)
        num_valid_images = 5
    else:
        if FLAGS.worst_attack:
            saving_dir = main_dir+'/Results/Imagenet/BOBY_'+str(FLAGS.eps)+'_2nd_500'+str(FLAGS.batch)+'_WORST_ATTACK.txt'
            print(saving_dir)
            saving_dir_full = main_dir+'/Results/Imagenet/BOBY_'+str(FLAGS.eps)+'_2nd_500'+str(FLAGS.batch)+'_WORST_ATTACK.txt'
        else:
            saving_dir = main_dir+'/Results/Imagenet/BOBY_'+str(FLAGS.eps)+'_2nd_500'+str(FLAGS.batch)+'.txt'
            print(saving_dir)
            saving_dir_full = main_dir+'/Results/Imagenet/BOBY_'+str(FLAGS.eps)+'_2nd_500'+str(FLAGS.batch)+'.txt'
        already_done = 0
        ...
        while not attack_completed:
            ...
            with tf.Session(config=session_conf) as sess:
                ...
                if first_iter:
                    ...
                    if FLAGS.worst_attack:
                        worst_class = np.argmin(logit_pr)
