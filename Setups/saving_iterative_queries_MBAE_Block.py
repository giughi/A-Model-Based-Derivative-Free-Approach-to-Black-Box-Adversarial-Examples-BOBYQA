...
python Setups/saving_iterative_queries_MBAE_Block.py --batch=25 --max_queries=1400 --test_size=20
"""
import time
import sys
sys.path.append("./")
...
flags.DEFINE_bool('hier', True, 'Hierarchical.')
flags.DEFINE_integer('max_steps', 10000, 'Maximum number of iterations')
flags.DEFINE_integer('max_queries', 15000, 'Maximum number of queries')
...
#         logger = utils.ResultLogger(FLAGS.output_dir, FLAGS.flag_values_dict())
    saving_dir = main_dir+'/Results/Imagenet/Iterative_'+str(FLAGS.eps)+'_batch_dim_'+str(FLAGS.batch)+'_max_queries_'+str(FLAGS.max_queries)+'_hier_'+str(FLAGS.hier)+'_2.txt'
    print(saving_dir)
    saving_dir_full = main_dir+'/Results/Imagenet/Iterative_'+str(FLAGS.eps)+'_batch_dim_'+str(FLAGS.batch)+'_max_queries_'+str(FLAGS.max_queries)+'_hier_'+str(FLAGS.hier)+'_2.txt'
    already_done = 0
    ...
    for ii in range(already_done, num_valid_images):
        ...
        while not attack_completed:
            ...
            with tf.Session(config=session_conf) as sess:
                ...
                attack = BlackBox_BOBYQA(sess, model, batch_size=FLAGS.batch , max_iterations=10000, print_every=1, 
                                        early_stop_iters=100, confidence=0, targeted=True, use_log=True,
                                        use_tanh=False, use_resize=FLAGS.hier, L_inf=FLAGS.eps, ordered_domain=True,
                                        image_distribution=True, mixed_distributions=False, max_eval=FLAGS.max_queries,
        ...
        if not right_classification:
            continue
        global_summary.append(attack_summary)
