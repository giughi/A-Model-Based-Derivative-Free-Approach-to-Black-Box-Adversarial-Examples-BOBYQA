...
from Attack_Code.BOBYQA.BOBYQA_Attack_BLOCKS_sim_COMBINATORIAL_adv_inception_random import BlackBox_BOBYQA
...
#         logger = utils.ResultLogger(FLAGS.output_dir, FLAGS.flag_values_dict())
    if FLAGS.print_image:
        ...
        if FLAGS.worst_attack:
            ...
            saving_dir = main_dir+'/Results/Imagenet/ADV_BOBY_'+str(FLAGS.eps)+'_2nd_500'+str(FLAGS.batch)+'.txt'
            print(saving_dir)
            saving_dir_full = main_dir+'/Results/Imagenet/ADV_BOBY_'+str(FLAGS.eps)+'_2nd_500'+str(FLAGS.batch)+'.txt'
        already_done = 0
