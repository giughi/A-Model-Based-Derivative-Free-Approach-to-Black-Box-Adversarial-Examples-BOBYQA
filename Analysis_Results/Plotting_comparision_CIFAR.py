...
for i in range(len(L_inf_var)):
    ...
    for j in range(np.minimum(np.minimum(len(dist_adv), len(gene_adv)), 
        ...
        block_tot_adv.append(block_adv[j][0])    
        
        if dist_adv[j][0]< maxiter:
            list_dist_2_adv.append(dist[j][0])
    ...
    SR_b_adv.append(len(list_dist_2_adv)/len(dist_tot_adv))
    SR_g_adv.append(len(list_gene_2_adv)/len(gene_tot_adv))
    SR_c_adv.append(len(list_combi_2_adv)/len(combi_tot_adv))
    SR_s_adv.append(len(list_block_2_adv)/len(block_tot_adv))
...
list_arrays = [list_gene_2,list_dist_2,list_combi_2,
