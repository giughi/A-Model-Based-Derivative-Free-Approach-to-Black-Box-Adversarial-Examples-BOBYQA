        ...
        for iteration in range(self.max_iters):
            ...
            for ibatch in range(num_batches):
                ...
                if len(success_indices) > 0:
                    ...
                    print('######## Successful at lazy insertion')
                    return noise, num_queries, curr_loss, True
            ...
            if tot_num_queries+num_queries>self.max_queries:
                    # noise[0, ...] = noise_batch[0, ...] # <================ THis is not the best noise, i
                    # curr_loss = losses[0]
                    print('######## internal Iterations insertion', bend-bstart, ' of ', len(noise_batch))
            ...
            while len(priority_queue) > 0:
                ...
                # print('The length of the label batch is ', len(label_batch))
                losses, preds = sess.run((self.losses, self.preds), feed_dict={self.test_in: image_batch,
                                                                               self.y_input: label_batch})
                num_queries += 1
                margin = losses[0]-curr_loss
                # print('test', len(priority_queue), margin,losses[0])
                ...
                if len(priority_queue) == 0 or margin <= priority_queue[0][0]:
                    ...
                    if self.targeted:
                        if preds == label:
                            print('####### succcesful in the heap  with preds', preds)
                            return noise, num_queries, curr_loss, True
                    else:
                        if preds != label:
                            return noise, num_queries, curr_loss, True
            ...
            if tot_num_queries+num_queries>self.max_queries:
                    ...
                    # curr_loss = losses[0]
                    print('######## internal Iterations deletion', bend-bstart, ' of ', len(noise_batch))
                    return noise, num_queries, curr_loss, False
