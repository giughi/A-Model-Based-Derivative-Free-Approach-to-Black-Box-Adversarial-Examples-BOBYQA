...
from Attack_Code.Combinatorial.attacks.local_search_helper_function import LocalSearchHelper


def ParsimoniousAttack_function(model, image, label, sess, values,max_internal_queries, noise,
                                y_input, test_in, losses, preds, _perturb_image,args, **kwargs):
    ...
    while True:
        ...
        for i in range(initial_batch, num_batches):
            ...
            noise, queries, loss, success = LocalSearchHelper(model, image, noise, label, sess, 
                                blocks_batch, num_queries, y_input, test_in, losses, preds,  
                                _perturb_image, args)
            time_end = time.time()
            num_queries += queries
