...
def divide_indice(vec, k):
    """
    Divide the index into k classes of sameish dimension
    """
    n = len(vec[vec])
    numb = np.arange(n)
    succ = np.random.choice(n, n, replace=False)
    appartening = []
        
    for j in range(k-1):
        elems = succ[int(j*np.floor(n/k)):int((j+1)*np.floor(n/k))]
        # print(j,np.floor(n/k),elems)
        appartening.append(np.isin(numb,elems))
    elems = succ[int((k-1)*np.floor(n/k)):]
    appartening.append(np.isin(numb,elems))
    
    return appartening

def matr_subregions_division_random(partition, k):
    """
    This allows to compute the matrix fo dimension equal to the image with the
    region of beloging for each supervariable with only a random composition
    :param partition: assigned Image that we are perturbin. This will have shape (1,m,m,3) 
    and values between the 1 and m, m being the number of variables. 
    :param k: number of times that we want to multiply m (of how much we want to increase the domain)
    :return: The matrix with the supervariable tto which each pixel belongs
    """
    
    n = int(np.max(partition))
    # print('===',n)
    A = partition.reshape(-1,)
    count = 0
    for i in range(n+1):
        # print(i)
        index = finding_indices(A,i)
        # print(index)
        assignation_vector = divide_indice(index, k)
        for j in range(k-1):
            ind = np.nonzero(index)[0]
            ind = ind[assignation_vector[j]]
            # print(A[ind], n+count+1)
            A[ind] = n+count+1
            count += 1
    # print(partition)
    return np.array([A.reshape(partition.shape)], dtype=np.int)
...
class BlackBox_BOBYQA:
    ...
    def attack_batch(self, img, lab):
        ...
        super_dependency = np.zeros(np.array([img]).shape)
        ...
        for step in range(self.steps_done+1, self.MAX_ITERATIONS):
            ...
            if self.ordered_domain:  
                ...
                if  force_renew:  
                    ...
                    super_dependency = matr_subregions_division_random(super_dependency, 
                                                                       int(self.use_var_len/(np.max(super_dependency)+1)))
