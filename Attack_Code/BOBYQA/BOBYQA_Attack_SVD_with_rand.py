...
def vec2modMatRand_Multi(c, indice, var, RandMatr, depend, b, a, Rand_El):
    temp = var.copy()
    n = len(indice)
    # print('shape of temp',temp.shape)
    # print('shape of rand',RandMatr.shape)
    # count = 0
    _, multi = Rand_El.shape
    ...
    for i in range(n):
        ...
        for j in range(multi):
            rand_el = Rand_El[i, j]
            indices_rand = depend == rand_el
            indices = np.logical_or(indices, indices_rand)
        # count+= len(RandMatr[depend == indx])
        # print('temp_size = ', len(temp[depend == indx])/temp.size)
        temp[indices] += c[i]*RandMatr[indices]
    # we have to clip the values to the boundaries
    # print('c',c)
    # print('total pixels', count/var.size)
    # print('nonzero in temp',np.count_nonzero(temp)/var.size)
...
def matr_subregions_division(var,n):
    ...
    for i in range(m):
        ...
        cl = np.random.choice(len(list), 1, replace=False, p=prob[list])
        A[i] = list[cl]
        mm -= 1
        count[list[cl]] += 1
        if count[list[cl]] == Tot[list[cl]]:
            list = np.delete(list,cl)
...
def SVR_assignment(img,n1):
    ...
    for i in range(n_cycle):
        ...
        Values[temp > Values] = temp[temp>Values] #U[idx_u]@S[idx_s]@Vh[idx_v]
...
def pos_ordering_matrix_2(prec,A,k,Z):
    ...
    while dummy:
        ...
        if Z[list(np.array(np.where(A == unique[i])).T.reshape(-1, 2, 1)[0])] < 0:
...
def find_highest_elem(prec,d,M,A):
    orde, pos, skip = pos_ordering_matrix_2(prec,M,d,A)   # TO DO: CHANGE D ITERATIVELY AS WHEN THE ITERATION ICREASES WE HA
    # skip = 0
    index = []
    for j in range(d):
...
def SVR_assignment_equal(img,n1):
    n = n1*n1*3
    mm = int(img.size*1)# /2?
    Assign = -1*np.ones(img.shape)
    Values = np.zeros(img.shape)
    # We first have to find the the different basis
    _,U,S,Vh = Hier_Svd(img)
    nn = len(S[0][S[0]>0]) + len(S[1][S[1]>0]) + len(S[2][S[2]>0])
    ...
    for i in range(n):
        ...
        while assigned<d:
            ...
            index, done = find_highest_elem(done, to_assign, M, Assign[:, :, col])
            index = expand_index(index, col)
            assigned += to_assign
            done_channel[col] += to_assign
            for k in range(len(index)):
                # print(len(Assign[index[k]]),'index',list(index[k]),'Assign',Assign[list(index[k])])
                Assign[list(index[k])] = i
...
def find_highest_elem_2(prec,d,M,A):
    orde, pos, skip = pos_ordering_matrix_2(prec, np.abs(M), d, A)
    # skip = 0
    signs = []
    index = []
    for j in range(d):
...
# def assign_capacities(pos,nn,mm):
#     availability_per_channel = np.zeros((3,))
#     for j in range(nn):
#         # updatae availability
#         availability_per_channel[pos[j][0]] += 1
#     assigned = np.ceil(mm/np.sum(availability_per_channel)*availability_per_channel)
#     return assigned
...
def SVR_assignment_equal_2(img, n1):
    ...
    for i in range(n):
        ...
        while assigned<d:
            ...
            # print('not asssigned', not_assigned, ' with space ', space_in_the_Svd)
            # print('to_assign',to_assign)
            index, done, signs = find_highest_elem_2(done, to_assign, M, Assign[:, :, col])
            index = expand_index(index, col)
            assigned += to_assign
            done_channel[col] += to_assign
            for k in range(len(index)):
...
class BlackBox_BOBYQA:
    ...
    def blackbox_optimizer_ordered_domain(self, iteration, ord_domain, Random_Matrix, super_dependency,img):
        ...
        if self.use_resize:
            ...
            index_set = set(indice)
            mask = np.array([(i in index_set) for i in range(len(ord_domain))])
            complementary_indices = ord_domain[~mask]
            multi = 1
            Rand_El = np.zeros((nn, multi))
            for i in range(nn):
                Rand_El[i, :] = complementary_indices[np.random.choice(len(complementary_indices), multi)]
            ...
            for i in range(nn):
                ...
                indices = super_dependency.reshape(-1,) == indice[i]

                for j in range(multi):
                    rand_el = Rand_El[i, j]
                    indices_rand = super_dependency.reshape(-1,) == rand_el
                    indices = np.logical_or(indices, indices_rand)
        ...
        opt_fun = Objfun(lambda c: self.sess.run([self.loss], feed_dict={
            self.modifier: vec2modMatRand_Multi(c, indice, var, Random_Matrix, super_dependency, bb, aa, Rand_El)})[0])
        x_o = np.zeros(self.batch_size,)
        times[1] = time.time()
        times[2] = time.time()
        ...
        # adjust sample probability, sample around the points with large gradient
        # print(soln)
        nimgs = vec2modMatRand_Multi(soln.x, indice, var, Random_Matrix, super_dependency, bb, aa, Rand_El)
