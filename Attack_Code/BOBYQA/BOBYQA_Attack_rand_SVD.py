...
def vec2modMatRand_Random(c, indice, var, RandMatr, depend, b, a):
    temp = var.copy().reshape(-1, )
    n = len(indice)
    for i in range(n):
        indices = finding_indices(depend, indice[i]).reshape(-1, )
        indices_tensor = finding_indices_tensor(depend, indice[i])  # .reshape(-1, )
        # print(indices)#len(RandMatr.reshape(-1, )[indices]))
        # print('len of indices is', len(indices))
        # count+= len(RandMatr[depend == indx])
        # print('temp_size = ', len(temp[depend == indx])/temp.size)
        temp[indices] += c[i]*RandMatr[indices_tensor].reshape(-1, )
    # we have to clip the values to the boundaries
    # print('c',c)
    # print('total pixels', count/var.size)
    # print('nonzero in temp',np.count_nonzero(temp)/var.size)
...
def pos_ordering_matrix_random(A, k):
    """
    Returning the ordered list of elements in A and for each of them its location in A. If we want to select the
    highest value of A we will then select A[list(pos[0])] and so on. Here we do not care about the element being
    already assigned.
    :param A: Two dimensional tensor that we are trying to order
    :param k: number of elements to be assigned
    :return: ordered list and position list
    """
    max_num = np.minimum(A.size, 3*k)

    ordered = np.sort(np.abs(A).reshape(-1,))[::-1]
    unique = np.sort(np.unique(ordered))[::-1]

    p = unique[:max_num]/np.sum(unique[:max_num])# select from unique
    # print(max_num, k)
    selected = np.random.choice(max_num, k, replace=False, p=p)
    for i in range(k):
        if i == 0:
            pos = np.array(np.where(np.abs(A) == unique[selected[i]])).T.reshape(-1, 2, 1)
        else:
            # print(selected[i], k)
            pos = np.concatenate((pos, np.array(np.where(np.abs(A) == unique[selected[i]])).T.reshape(-1, 2, 1)))

    return ordered, pos, 0
...
def find_highest_elem_rand(prec, d, M):
    """
    This function finds d random pixels in M, sampling according to the distribution generated by the intensity
    of these pixels. Note that M is already a svd decomposition, hence the values are already giving an idea of
    how important the values are.
    :param prec: Number of vairables assigned before for the same SVD. Need it for consistency with the previous
    versions of the algorithm.
    :param d: Number of pixels that we have to assign.
    :param M: SVD decomposition of an image
    :return An array with the indexes of the elements that have been chosen, number of variables assigned within
    the SVD, signs of the variables at these locations:
    """
    orde, pos, skip = pos_ordering_matrix_random(M, d)
    # print('pos has length', len(pos))
    # skip = 0
    signs = []
    index = []
    for j in range(d):
...
def assign_index(Assig, index, el, trig):
    ...
    if dummy:
        ...
        temp.append(1)
        Increase = trig*np.ones(tuple(temp))
...
def finding_indices_tensor(dependency, index):
    return dependency == index# , axis=len(dependency.shape)-1)


def SVR_assignment_equal_2(img, n1):
    n = n1*n1*3
    mm = int(img.size*0.05)  # /2?
    shape1 = list(img.shape)
    shape1.append(1)
    shape = tuple(shape1)
    Assign = -1*np.ones(shape)
    Values = 0*np.ones(shape) #  np.ones(img.shape)
    # We first have to find the the different basis
    _, U, S, Vh = Hier_Svd(img)
    nn = int((len(S[0][S[0] > 0]) + len(S[1][S[1] > 0]) + len(S[2][S[2] > 0]))*0.05)
    ...
    print('Total of ', mm, ' pixels divided between ', nn, ' super variables; ', d, 'each')
    prec = 0
    done = 0
    assigned = 0
    ...
    for i in range(n):
        ...
        while assigned < d:
            ...
            if prec == 0:
                ...
                M = S[col][num] * np.outer(U[:, num, col], Vh[num, :, col])

            not_assigned = d-assigned
            space_in_the_Svd = max_m - prec
            to_assign = int(np.minimum(not_assigned,
                                   space_in_the_Svd))
            rem = to_assign # mm/3-done_channel[col] This is because in the repeated case we can have all the pixels assigned more than once
            to_assign = int(np.minimum(to_assign, rem))
            # print('Sing Value number ',c,' in channel ', col,' with remainig', rem,' to do ',d, 'to assign', to_assign)
            index, done, signs = find_highest_elem_rand(prec, to_assign*2, M)
            ...
            for k in range(len(index)):
                ...
                Assign = assign_index(Assign, list(index[k]), i, -1)
                Values = assign_index(Values, list(index[k]), signs[k], 0) #  [list(index[k])] = signs[k]
    ...
    def blackbox_optimizer_ordered_domain(self, iteration, ord_domain, Random_Matrix, super_dependency, img):
        ...
        if self.use_resize:
            ...
            for i in range(nn):
                ...
                indices_tensor = finding_indices_tensor(super_dependency, indice[i])#.reshape(-1, )

                print(' Length of Random is ', len(Random_Matrix[indices_tensor].reshape(-1,)))
                print(' Lenght of modifier is ', len(self.modifier_up[indices]))

                up = np.maximum(self.modifier_up[indices]*Random_Matrix[indices_tensor].reshape(-1,),
                                self.modifier_down[indices]*Random_Matrix[indices_tensor].reshape(-1,))

                down = np.minimum(self.modifier_down[indices]*Random_Matrix[indices_tensor].reshape(-1,),
                                  self.modifier_up[indices]*Random_Matrix[indices_tensor].reshape(-1,))
                a[i] = np.min(down)
                b[i] = np.max(up)
        else:
            b = self.modifier_up[indice]
        ...
        print('max is ', np.max(b-a), 'min', np.min(b-a))
        # times[1] = time.time()
        opt_fun = Objfun(lambda c: self.sess.run([self.loss], feed_dict={
            self.modifier: vec2modMatRand_Random(c, indice, var, Random_Matrix, super_dependency, bb, aa)})[0])
        x_o = np.zeros(self.batch_size,)
        times[1] = time.time()
        times[2] = time.time()
        ...
        print(soln)
        nimgs = vec2modMatRand_Random(soln.x, indice, var, Random_Matrix, super_dependency, bb, aa)
                ...
                if (np.mod(iteration_scale, 50) == 0) or force_renew:
                    ...
                    if first_renew:
                        ...
                        print('In the superdependency there are ', len(super_dependency[super_dependency > -1]),
                              'elements assigned')
                        print(' While in the random matrix the maximum is ', np.max(Random_Matrix), ' and the minimum ',
                              'is ', np.min(Random_Matrix))
