...
def vec2modMatRand3(c, indice, var, RandMatr, depend, b, a, overshoot):
    ...
    centered.
    """
    # print('Within MatRand3 len(indice):', len(indice),', len(c):', len(c))
...
class BlackBox_BOBYQA:
    ...
    def blackbox_optimizer_ordered_domain(self, iteration, ord_domain, Random_Matrix, super_dependency, img, k, img0):
        ...
        if self.use_resize:
            ...
            for i in range(nn):                
                ...
                xs =  np.divide( -(up+down),
                                (up-down))
                s_o[i] = np.clip(np.arctanh(np.clip(xs[max_ind],-1,1)),-3,3)
                if i == 0:
                    x_o[i] = 3
        ...
        # print('nn',nn)
                ...
                S_n = np.concatenate((s_o.reshape(1,nn)/3,
                                     np.random.randn(nn_var-1,nn)/np.sqrt(nn_var))
                                    ).transpose()
        ...
        # print(x_o)
        # print(np.dot(S,x_o))
        opt_fun = Objfun(lambda c: self.loss_f(img, vec2modMatRand3(np.dot(S,c), indice, var, Random_Matrix, super_dependency, bb, aa,
        ...
        soln = pybobyqa.solve(opt_fun, x_o, rhobeg=np.min(b-a)/3,
                              bounds=(a, b), maxfun=nn_var*1.5,
                              rhoend=np.min(b-a)/6,
                              npt=nn_var+1, scaling_within_bounds=False,
                              user_params=user_params)
        summary = opt_fun.get_summary(with_xs=True)
        minimiser = np.min(summary['fvals'])
        # print(soln)
        real_oe = self.loss_f(img, vec2modMatRand3(np.dot(S,soln.x), indice, var, Random_Matrix, super_dependency, bb, aa,
        ...
        nimgs = vec2modMatRand3(np.dot(S,soln.x), indice, var, Random_Matrix, super_dependency, bb, aa, self.overshoot)
        nimg2 = nimgs.copy()
