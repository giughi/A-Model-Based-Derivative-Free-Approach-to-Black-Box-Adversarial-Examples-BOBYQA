    ...
    def attack_batch(self, img, lab):
        ...
        for step in range(self.MAX_ITERATIONS):
            ...
            if step%(self.print_every) == 0:
                loss, output, distance = self.sess.run((self.loss,self.output, self.distance), feed_dict={self.modifier: self.real_modifier})
            ...
            if not self.use_tanh:# and (step>0):
                scaled_modifier = self.sess.run([self.scaled_modifier],feed_dict = {self.modifier: self.real_modifier})[0]
            ...
            if self.ordered_domain:  
                ...
                if self.use_resize:
                    ...
                    if step == 3:#30:#(resize_iter == 1) and (previous_#loss == 1e6):#
                        self.small_x = 10
                        self.small_y = 10
                    ...
                    if step == 55:#140:#(resize_iter == 2) and (previous_loss == 1e6):#step == 100:
                    ...
                    if step == 130:#400:#(resize_iter == 3) and (previous_loss == 1e6):#step == 200:
                        # print('Now we will use dimension 64')
                        self.small_x = 67
                        self.small_y = 67
                        self.resize_img(self.small_y, self.small_x, False)
            ...
            loss_test, score, real, other = self.sess.run((self.loss,self.output,self.real,self.other), feed_dict={self.modifier: nimg})
