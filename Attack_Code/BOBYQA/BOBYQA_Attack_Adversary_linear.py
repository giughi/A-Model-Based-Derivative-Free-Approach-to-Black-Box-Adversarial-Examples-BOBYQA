"""
We introduce a linear interpolation rather than the block one
"""
...
def vec2modMatRand3(c, indice, var, b, a, bb, aa, image_size, pert_size):
    """
    ...
    for i in range(n):
        indices = indice[i]
        # print('n',n)
        temp[indices] += (c[i]*(b-a)[i]/2 + (b+a)[i]/2)
    # we have to clip the values to the boundaries
    temp = temp.reshape((pert_size, pert_size, 3))
    temp = cv2.resize(temp, (image_size, image_size), 
                            interpolation=cv2.INTER_LINEAR)
    temp = np.minimum(bb.reshape(-1, ), temp.reshape(-1, ))
    temp = np.maximum(aa.reshape(-1, ), temp)
    
    return temp.reshape((image_size, image_size,3))
...
class BlackBox_BOBYQA:
    ...
    def blackbox_optimizer_ordered_domain(self, iteration, ord_domain, Random_Matrix, super_dependency, img, k, img0):
        ...
        eta_hat = np.array(cv2.resize((img-img0)[0], (self.small_x, self.small_y), 
        ...
        mod_up_hat = cv2.resize(mod_up, (self.small_x, self.small_y), 
                            interpolation=cv2.INTER_LINEAR).reshape(-1,)
        mod_down = self.modifier_down.reshape((self.size_img, self.size_img,3))
        ...
        if self.use_resize:
            ...
            for i in range(nn):
                ...
                x_o[i] = np.clip(eta_hat[indice[i]]/(
                                        mod_up_hat[indice[i]]-mod_down_hat[indice[i]])*2,
                                 -1,1)
                # print(x_o[i])
        ...
        # print(indice)img
        opt_fun = Objfun(lambda c: self.loss_f(img, [vec2modMatRand3(c, indice, eta_hat, 
                                                        b, a, bb,aa, self.image_size, self.small_x)], 
                                                    only_loss=True)[0])
        ...
        nimgs = vec2modMatRand3(soln.x, indice, eta_hat, b, a, bb,aa, self.image_size, self.small_x)        
        ...
        if soln.f > initial_loss:
            ...
            return distance2[0], evaluations + 2, np.array([nimg2]), times, summary
        else:
            # print('The optimisation is working. THe diff is ', initial_loss - soln.f)
            return distance[0], evaluations + 2, np.array([nimgs]), times, summary
    ...
    def attack_batch(self, img, lab):
        ...
        for step in range(self.steps_done+1, self.MAX_ITERATIONS):
            ...
            loss, output, distance = self.loss_f(img, 0* np.array([adv]))
            score = output[0]
            print('LOSS AFTER ATTACK new kind', loss)
            loss, output, distance = self.loss_f(img, np.array([adv]))
            score = output[0]
            print('LOSS AFTER ATTACK', loss)
