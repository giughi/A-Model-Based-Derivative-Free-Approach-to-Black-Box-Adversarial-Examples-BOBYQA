...
class GenAttack2(object):
    ...
    def attack_step(self, idx, success, orig_copies, cur_noise, prev_elite, margin_log, best_win_margin, cur_plateau_count, num_plateaus):
        ...
        win_margin = tf.reduce_max(pop_preds[:, self.target] - tf.reduce_max(pop_preds, axis=1))
        ...
        parents = tf.distributions.Categorical(probs=select_probs).sample(2*self.pop_size-2)
        ...
        return idx, success, orig_copies, new_pop, tf.reshape(elite,(noise_dim, noise_dim, 3)), margin_log, new_best_win_margin, new_cur_plateau_count, new_num_plateaus
    ...
    def __init__(self, model, pop_size=6, mutation_rate=0.001,
        ...
        self.tlab = tf.contrib.layers.one_hot_encoding([self.target], num_classes=1001)
        ...
        cond = lambda  i, success, pop_orig, pop_noise, cur_elite, margin_log, best_win_margin, cur_plateau_count, num_plateaus: tf.logical_and(tf.less_equal(i, self.max_steps), tf.equal(success, 0))


        attack_body = lambda i, success, pop_orig, pop_noise, cur_elite, margin_log, best_win_margin, cur_plateau_count, num_plateaus: self.attack_step(i, success, pop_orig, pop_noise, cur_elite, margin_log, best_win_margin, cur_plateau_count, num_plateaus)
        
        
        self.attack_main = tf.while_loop(cond, attack_body, [self.i, self.init_success,  self.pop_orig,
             self.pop_noise, self.pop_noise[0], self.margin_log, self.best_win_margin, self.cur_plateau_count,
             self.num_plateaus])
    ...
    def attack(self, sess, input_img, target_label): 
        ...
        (num_steps, success,  copies, final_pop, adv_noise, log_hist,_,_,_) = sess.run(self.attack_main)
        if success:
            if self.resize_dim:
                adv_img = sess.run(
                    tf.clip_by_value(
            ...
            return adv_img[0], query_count, log_hist[1:,:], success
        else:
            return num_steps, num_steps, num_steps, False
