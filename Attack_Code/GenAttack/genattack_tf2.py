...
class GenAttack2(object):
    ...
    def attack_step(self, idx, success, orig_copies, cur_noise, prev_elite, best_win_margin, cur_plateau_count, num_plateaus):
        if self.resize_dim:
            noise_resized = tf.image.resize_bilinear(cur_noise, (299,299))
        else:
        ...
        print('############## the curreent noise is', cur_noise)
        print('############## the curreent noise shape', cur_noise.shape)
        ...
        print('###########################', select_probs, select_probs.shape)
        ...
        pp2 = pp2 / (pp1+pp2)
        pp2 = tf.tile(tf.expand_dims(tf.expand_dims(tf.expand_dims(pp2, 1), 
                                                    2), 
                                     3), 
                      (1, noise_dim, noise_dim,3))
        print(' ========================= ', parent1)
        prob = tf.random_uniform(shape=parent1.get_shape())
        xover_prop = tf.cast( prob > pp2, tf.float32)
...
#         margin_log = tf.concat([margin_log, [[win_margin]]], axis=0)
    ...
    def __init__(self, model, pop_size=6, mutation_rate=0.001,
        ...
        self.input_img = tf.Variable(np.zeros((1, 299,299,3), dtype=np.float32), name='x', dtype=tf.float32)
        # copies of original image
        self.pop_orig = tf.Variable(np.zeros((self.pop_size, 299, 299, 3), dtype=np.float32), name='pop_orig', dtype=tf.float32)
        ...
        self.box_min = tf.tile(tf.maximum(self.input_img-eps, -0.5), (self.pop_size, 1,1,1))
        self.box_max = tf.tile(tf.minimum(self.input_img+eps, 0.5), (self.pop_size, 1,1,1))
...
#         self.margin_log.set_shape((None,1))
        self.tlab = tf.contrib.layers.one_hot_encoding([self.target], num_classes=num_classes)
...
#         sess.run(tf.assign(self.margin_log, np.zeros((1,1), dtype=np.float32)))
    ...
    def attack(self, sess, input_img, target_label): 
        ...
        (num_steps, success,  copies, final_pop, adv_noise,_,_,_) = sess.run(self.attack_main)
        if success:
            if self.resize_dim:
                adv_img = sess.run(
                        tf.clip_by_value(
                            np.expand_dims(input_img, axis=0)+tf.image.resize_bilinear(np.expand_dims(adv_noise, axis=0), (299,299)),
